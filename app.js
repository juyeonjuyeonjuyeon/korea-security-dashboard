...
const matchNews=(data,pattern,id)=>{
  ...
  return [...direct,...matched].filter((item,index,all)=>all.findIndex(other=>other.url===item.url&&other.source===item.source)===index).slice(0,id==="embassy_withdrawal"?8:2);
};
...
if("serviceWorker"in navigator){let reloading=false;navigator.serviceWorker.addEventListener("controllerchange",()=>{if(!reloading){reloading=true;location.reload()}});navigator.serviceWorker.register("./sw.js?v=22")}
